...
template <typename T>
GddSegment<T>::GddSegment(const std::shared_ptr<const pmr_vector<T>>& dictionary,
...
template <typename T>
std::shared_ptr<const pmr_vector<T>> GddSegment<T>::dictionary() const {
...
std::shared_ptr<AbstractSegment> GddSegment<T>::copy_using_allocator(
  ...
  auto copy = std::make_shared<GddSegment<T>>(std::move(new_dictionary), std::move(new_attribute_vector));
...
template <typename T>
ValueID GddSegment<T>::lower_bound(const AllTypeVariant& value) const {
...
template <typename T>
ValueID GddSegment<T>::upper_bound(const AllTypeVariant& value) const {
  DebugAssert(!variant_is_null(value), "Null value passed.");
  access_counter[SegmentAccessCounter::AccessType::Gdd] +=
      static_cast<uint64_t>(std::ceil(std::log2(_dictionary->size())));
  const auto typed_value = boost::get<T>(value);
...
template <typename T>
AllTypeVariant GddSegment<T>::value_of_value_id(const ValueID value_id) const {
...
template <typename T>
ValueID::base_type GddSegment<T>::unique_values_count() const {
  return static_cast<ValueID::base_type>(_dictionary->size());
}

template <typename T>
std::shared_ptr<const BaseCompressedVector> GddSegment<T>::attribute_vector() const {
  return _attribute_vector;
}

template <typename T>
ValueID GddSegment<T>::null_value_id() const {

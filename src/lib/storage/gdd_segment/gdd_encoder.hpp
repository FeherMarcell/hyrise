 ...
 public:
  ...
  std::shared_ptr<AbstractEncodedSegment> _on_encode(const AnySegmentIterable<T> segment_iterable,
    ...
    segment_iterable.with_iterators([&](auto segment_it, auto segment_end) {
      ...
      while (segment_it != segment_end) {
        ...
        if (block_contains_values) {
          // GDDTODO: std::make_unsigned is not compatible with float and double, which are supported by Gdd
          /*
    ...
    if (segment_contains_null_values) {
      return std::make_shared<GddSegment<T>>(std::move(block_minima), std::move(null_values),
                                                          std::move(compressed_offset_values));
    }
    return std::make_shared<GddSegment<T>>(std::move(block_minima), std::nullopt,
                                                        std::move(compressed_offset_values));
  }
};

...
#include "storage/gdd_segment/v1-fixed/gdd_segment_iterable.hpp"
...
auto create_iterable_from_segment(const GddSegmentV1Fixed<T, Enabled>& segment) {
  ...
  return AnySegmentIterable<T>(GddSegmentV1FixedIterable<T>(segment));
#else
  if constexpr (EraseSegmentType) {
    return create_any_segment_iterable<T>(segment);
  } else {
    return GddSegmentV1FixedIterable<T>{segment};
  }
#endif
}
